...
def get_test_data():
    ...
    with open(data_directory+'csv_files/OhioT1DM-testing/imputed/multivariate_continuous_subsets.pickle', 'rb') as f: #data/OhioT1DM-testing/processed/multivariate_continuous_subsets.pickle
    ...
    for subj in unpickled_data.keys():
        ...
        for key in keys:
            ...
            values = df.values
            windowed_data = pd.concat([windowed_data, series_to_supervised(values, history_window, prediction_window)])
...
if __name__ == "__main__":
    ...
    for subject in test_subjs:
        ...
        dc_ID = subject
        filename = dc_ID + '_'+PH+'.txt'
        merged_df.to_csv(results_directory+filename,  index=None, sep=' ', mode='w')
